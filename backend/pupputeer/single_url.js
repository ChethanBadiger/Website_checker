...
async function checkSite(url, outputDir) {
    ...
    try{
        ...
        if(/(captcha|verify you are human|are you a robot|bot checker|cloudflare|hcaptcha|recaptcha)/i.test(content)) {
            res.anomaly = true;
...
(async () => {
    const result = await checkSite('https://jperm.net', './results');
    console.log(result);
})();